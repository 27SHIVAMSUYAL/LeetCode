...
public:
    int areaOfMaxDiagonal(vector<vector<int>>& dimentions) {
        ...
        double maxDiagonal = sqrt(dimentions[0][0]*dimentions[0][0] + dimentions[0][1]*dimentions[0][1]);
        int maxArea = dimentions[0][0]*dimentions[0][1];

        for(int i = 0 ; i < dimentions.size();i++){

            double dia = sqrt(dimentions[i][0]*dimentions[i][0] + dimentions[i][1]*dimentions[i][1]);
